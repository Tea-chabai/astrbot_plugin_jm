...
| 指令 | 行为 |
| --- | --- |
| `/jm <编号>` | 下载整本，转换为 PDF 发送 |
| `/jm <编号> <章号>` | 只下载多章节本子中的某一章，例如 `/jm 553653 2` |
| `/jmhelp` | 查看帮助 |

- **单章节作品** —— 一个 PDF
- **多章节作品** —— 默认拒绝；开启多章节下载后整本下载，每章一个 PDF，
  整体打包成 ZIP

编号不存在、或需要登录而当前未登录时，回复「未查询到相关本子」。

> **为什么单章要用参数而不是直接发章节编号？**
> JM 的专辑 ID 与第 1 章的 photo ID 是同一个值，而且用章节 ID 查专辑时返回的
> `album.id` 就是那个章节 ID。也就是说，光看一串数字无法判断用户想要整本还是
> 只要某一章。所以「单章」只能靠显式的章号参数表达。
...
- 所有章节打包成一个 ZIP 发送，ZIP 名形如 `标题_专辑编号.zip`；
  只取单章时包名会带章号，形如 `标题_章号_编号.zip`
- **整本**下载受 `max_chapters` 限制（默认 30 章）与 3000 张图片的硬性上限
- **单章**请求不受上述整本上限影响 —— 否则 132 章的大长篇连一章都取不了。
  但单章自身仍受 `max_images`（默认 500 张）限制
- 个别章节下载失败不会中断整本，失败章节会在回复里列出

### 密码保护

需要同时满足两个配置项：`encrypt_output`（开关，默认关闭）与
`lock_password`（密码，至少 4 位）。开启后：

- 发送的文件一律打包成 **ZIP 并用 WinZip AES-256 加密**，单章节也会打包
- **ZIP 里的 PDF 不单独加密**，解压后直接可读，不必每开一章输一次密码
- 密码会在回复消息中一并告知使用者
- 开启加密时，未加密的中间产物（单章的原 PDF）会在打包后删除，
  避免在输出目录里留下未加密副本
...
| `encrypt_output` | false | 是否加密输出。开启后所有产物打包成加密 ZIP |
| `lock_password` | 空 | 加密密码，至少 4 位。仅在开关开启时生效 |
...
- **多章节判定**：先取专辑详情 `get_album_detail()`，其 `episode_list` 长度大于 1
  即视为多章节。
- **章节由参数指定，不由 ID 推断**：JM 的专辑 ID 与第 1 章 photo ID 同值，且用
  章节 ID 查专辑时返回的 `album.id` 就是该章节 ID，因此无法从数字本身判断
  用户想要整本还是某章。单章只能用 `/jm <编号> <章号>` 表达。
...
- **逐章下载**：多章节走独立的 `_download_album_zip()`，逐章调用
  ...
  不会丢掉已下载的内容；每章下载后单独校验张数（`_validate_book` 只看得到
  首章，管不住后面异常长的章节）。
- **加密**：用 `pyzipper` 写 WinZip AES-256。标准库 `zipfile` 不支持写入加密
  包，而传统 ZipCrypto 较新的 Windows 资源管理器又打不开。只加密 ZIP 容器，
  内部 PDF 保持明文，避免每开一章输一次密码。未加密时退回标准库。
...
调小，或改用 `/jm <编号> <章号>` 只取需要的几章。

## 测试

`tests/run_test.py` 用假的 event 对象驱动插件的真实代码路径，不需要真正的
消息平台即可验证逻辑。脚本默认跑基础用例，多章节与加密需通过环境变量开启。
...
# 加密（单章也会被打包成加密 ZIP）
JM_TEST_ID=422866 JM_TEST_ENCRYPT=你的密码 python tests/run_test.py

# 多章节：整本、章号上限、只取单章、章号越界
JM_TEST_MULTI=1 JM_TEST_MAXCH=3 JM_TEST_LOCK=你的密码 python tests/run_test.py
```
